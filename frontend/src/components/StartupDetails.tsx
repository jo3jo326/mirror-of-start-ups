...
const StartupDetails: React.FC<StartupDetailsProps> = ({ startup }) => (
  <div className={styles.startupDetailsModalContent}>
    ...
    <div className={styles.startupRow}><span className={styles.startupLabel}>Pitch Video URL</span><span className={styles.startupColon}>:</span><span className={styles.startupValue}>{startup.pitchVideoUrl ? <a href={startup.pitchVideoUrl} target="_blank" rel="noopener noreferrer">{startup.pitchVideoUrl}</a> : '-'}</span></div>
